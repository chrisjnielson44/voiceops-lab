    ...
    <ChartContext.Provider value={{ config }}>
      <div
        ...
        className={cn(
          ...
          "[&_.recharts-radial-bar-background-sector]:fill-secondary",
          "[&_.recharts-rectangle.recharts-tooltip-cursor]:fill-secondary",
...
/**
 * Recharts injects these into the `content` render component. Typed locally so
 * the wrapper is independent of recharts' internal (version-specific) generics.
 */
  ...
  (
    ...
    return (
      <div
        ref={ref}
        className={cn(
          "grid min-w-[8rem] items-start gap-1.5 rounded-lg border border-border bg-popover px-2.5 py-1.5 text-xs shadow-pop",
          className,
        )}
      >
        {!nestLabel ? tooltipLabel : null}
        <div className="grid gap-1.5">
          {payload.map((item: Record<string, unknown> & { dataKey?: string; name?: string; value?: number; color?: string; payload?: Record<string, unknown> }, index: number) => {
